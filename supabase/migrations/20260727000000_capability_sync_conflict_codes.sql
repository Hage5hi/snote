-- Give the client an operation-safe conflict contract without rewriting
-- previous migrations. A generic version_conflict remains for legacy
-- note-manage transitions; sync RPCs must distinguish an encryption epoch
-- mismatch from the harmless checkpoint compare-and-swap race.
BEGIN;
...
BEGIN
  ...
  IF v_note.encryption_version <> p_expected_encryption_version THEN
    RETURN jsonb_build_object('status', 'append_encryption_conflict');
  END IF;
...
BEGIN
  ...
  END IF;
  IF v_note.encryption_version <> p_expected_encryption_version THEN
    RETURN jsonb_build_object('status', 'checkpoint_encryption_conflict');
  END IF;
  ...
  IF v_latest_version <> p_expected_checkpoint_version THEN
    RETURN jsonb_build_object('status', 'checkpoint_version_conflict');
  END IF;
...
REVOKE ALL ON FUNCTION public.capability_updates_append(text, jsonb, bigint)
  FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.capability_updates_append(text, jsonb, bigint)
  TO service_role;
