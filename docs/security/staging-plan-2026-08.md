...
- `bun run staging:prepare` creates a disposable OS-temp workdir containing
  Edge Function sources and an explicit allowlist of additive migrations;
- the generated config uses `snote-staging-local`, records migration SHA-256
  hashes, and rejects the production ref `onfzjmfjldsbthchssfr`;
...
disk capacity is sufficient, and a Supabase CLI version is pinned and recorded.
...
bunx supabase --workdir "<generated-workdir>" start
...
- all 22 migration filenames and SHA-256 hashes match generated files;
...
Build the local SPA with `VITE_CAPABILITY_ROUTES_ENABLED=true` and managed Auth
disabled. The API must therefore return polling sessions. G3B may claim only:
...
- migration filenames and SHA-256 hashes plus the applied ledger;
