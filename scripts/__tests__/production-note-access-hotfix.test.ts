...
describe("production note access hotfix", () => {
  it("keeps ordinary note entry points legacy-only unless staging opts in exactly", () => {
    ...
    expect(split).toContain("legacyOnly = true");
    expect(split).toContain("legacyOnly={legacyOnly}");
