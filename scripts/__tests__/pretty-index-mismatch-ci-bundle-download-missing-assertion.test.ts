...
// when `validate-schema-assertion.txt` is COMPLETELY MISSING from the
// extracted tarball (with `validate-report.json` present non-empty so
// the failure is scoped to exactly the missing assertion file). Pins
// the exact ERROR block wording — sibling to
// `pretty-index-mismatch-ci-bundle-download-missing.test.ts` (missing
// report) and `…-empty-assertion.test.ts` (assertion present but empty).
...
d("pretty-index-mismatch-ci-bundle-download — missing assertion snapshot", () => {
  ...
  it("fails with a stable ERROR block when validate-schema-assertion.txt is missing", () => {
    // Only validate-report.json present — isolates the failure to the
    // missing assertion file branch.
...
`;
    ...
    expect(stderr).toMatchInlineSnapshot(`
      "ERROR: extracted tarball ./_pi-ci-bundle-atomic/pi-ci-atomic.tar.gz failed content checks:
        - MISSING file: expected at ./_pi-ci-bundle-atomic/extracted/pi-ci-atomic/validate-schema-assertion.txt
        extracted tree:
